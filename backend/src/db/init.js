...
function seedMaintenanceCategories() {
    const count = db.prepare('SELECT COUNT(*) as c FROM maintenance_categories').get().c;
    if (count > 0) return;
    const now = Date.now();
    const insert = db.prepare(`
    ...
    `);
    const defaults = [
        ['auth',          'Authentication',   'Account login, registration, JWT and Discord OAuth.',           1, null, 0, now],
        ['store',         'Store',            'Browsing products, viewing details, filters and search.',      1, null, 1, now],
        ['payments',      'Payments & Bank',  'Crypto payments, LC bank, orders and licenses.',                1, null, 2, now],
        ['launcher',      'Desktop Launcher', 'Downloading and running the UCHIHA-Launcher desktop app.',      1, null, 3, now],
        ['forum',         'Forum & Q&A',      'Community forum posts, replies, comments and Q&A entries.',     1, null, 4, now],
        ['discord_tools', 'Discord Tools',    'Discord automation products and license activation.',          1, null, 5, now],
    ];
    const tx = db.transaction((rows) => { for (const r of rows) insert.run(...r); });
    tx(defaults);
    console.log('[db] seeded maintenance categories');
}
