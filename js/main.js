...
async function downloadLauncher() {
    ...
    if (isElectron || !isHttps) {
        ...
        var assetUrl = API_BASE_RAW + '/api/launcher/download?platform=windows';
        var banner = document.createElement('div');
...
function buildProductCard(p, showCategory) {
    ...
    if (isFree) {
        ...
        if (p.file_product === false) {
            ...
            dlBtn.className = 'product-btn product-btn-download';
            dlBtn.innerHTML = '<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg> Download ' + escHtml(p.file_name || 'File');
            dlBtn.addEventListener('click', function(e) {
                e.stopPropagation();
                var a = document.createElement('a');
                a.href = apiUrl(p.file_url);
                a.download = p.file_name || 'download';
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
            });
